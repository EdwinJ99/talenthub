...
export default function DraftPage() {
  ...
  const handleGenerateQuotation = async () => {
    ...
    await showSuccess("Berhasil", "Quotation berhasil dibuat.");

    router.push("/tracking/quotation");
  };

  const handleDelete = async (creatorNo: number) => {
    const result = await confirmDelete("Hapus Creator?");

    if (!result.isConfirmed) return;

    setCreators((prev) => prev.filter((item) => item.no !== creatorNo));

    await showSuccess("Berhasil", "Creator berhasil dihapus.");
  };
  ...
  const stepDates = [projectDetail.date, "-", "-", "-", "-", "-"];
      ...
      <div className="space-y-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <FieldBox label="Project Name" value={projectDetail.projectName} />

            <FieldBox label="PIC" value={projectDetail.pic} />

            <FieldBox label="Date" value={projectDetail.date} />
          </div>
        </section>
