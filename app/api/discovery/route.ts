...
export async function GET(request: NextRequest) {
  try {
    ...
    if (social_media) where.social_media = { equals: social_media, mode: "insensitive" };
    ...
    if (category) {
      if (!isNaN(Number(category))) {
        ...
        where.mst_categories = { name: { equals: category, mode: "insensitive" } };
      }
    }
