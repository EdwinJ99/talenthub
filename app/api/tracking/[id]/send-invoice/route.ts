...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isInteger(projectId)) return NextResponse.json({ error: "ID project tidak valid" }, { status: 400 });

  const smtpUser = process.env.SMTP_USER;
  const smtpPassword = process.env.SMTP_PASSWORD;
  if (!smtpUser || !smtpPassword) return NextResponse.json({ error: "Konfigurasi SMTP belum tersedia di server" }, { status: 500 });
  ...
  try {
    ...
    if (!recipientEmail) return NextResponse.json({ error: "Email brand belum diisi" }, { status: 400 });

    const formData = await request.formData();
    const invoice = formData.get("invoice");
    if (!(invoice instanceof File) || invoice.size === 0) return NextResponse.json({ error: "File PDF invoice wajib disertakan" }, { status: 400 });
    ...
    return NextResponse.json({ error: "Gagal mengirim email invoice" }, { status: 500 });
  }
}
